    ...
    return (
        <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
                {[
                    { id: 'warehouses' as const, label: `Warehouses (${warehouses.length})`, icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg> },
                    { id: 'stock' as const, label: `Stock ${selectedWarehouse ? `- ${selectedWarehouse.name}` : ''}`, icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg> },
                    { id: 'transfers' as const, label: `Transfers (${transfers.length})`, icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg> }
                ].map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={activeTab === tab.id ? "btn-primary" : "btn-outline"} style={{ width: 'auto', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        {tab.icon} {tab.label}
                    </button>
                ))}
            </div>
